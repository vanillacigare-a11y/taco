...
(function () {
  ...
  var G = {
    ...
    climbChance: 0.55, sleepAfter: 20000, bubbleMs: 2600,
  ...
  var CHARS = [
    {
      id: 'a', name: '시메지', on: true, size: 96, speed: 1.2,
      pref: { rice: 1, snack: 2, veg: 0 },
      sprites: { stand: [], walk: [], run: [], sit: [], sleep: [], hungry: [], eat: [], fight: [], drag: [], fall: [], climb: [] },
      lines: {
        click:      ['왜 불러?', '응?', '심심해…', '오늘 글 잘 써져?'],
        dblclick:   ['꺄악!', '어지러워…', '한 번 더 해줘!'],
        pet:        ['헤헤', '좋아…', '더 해줘'],
        idle:       ['오늘 방문자 몇 명이지', '글 하나 더 써볼까', '…'],
        grab:       ['우와악', '내려줘…', '높아 높아'],
        throw:      ['날아간다!', '으아아'],
        landHard:   ['아야…', '방금 아팠어'],
        climb:      ['영차', '올라간다'],
        sleep:      ['zzz…', '조금만 잘게…'],
        wake:       ['앗 깼다', '음냐…'],
        foodSpotted:['밥이다!', '저거 내 거!'],
        eat:        ['냠냠', '맛있다', '우물우물'],
        full:       ['잘 먹었습니다!', '배불러~'],
        favorite:   ['이거 완전 내 최애야!', '이 맛이지…'],
        hateFood:   ['으… 이건 좀', '배고파서 먹는 거야'],
        foodLost:   ['아 뺏겼다', '내 밥이었는데'],
        greet:      ['안녕!', '뭐 해?'],
        reply:      ['어 안녕', '그냥 노는 중'],
        fightStart: ['그거 내 거야!', '비켜!'],
        fightTaunt: ['이거 놔!', '내가 먼저 봤어!'],
        fightWin:   ['내가 이겼다!', '역시 나지'],
        fightLose:  ['치사해…', '흥'],
        makeUp:     ['아까는 미안', '화해하자'],
        happy:      ['오늘 기분 좋아', '헤헤헤', '살 만해'],
        grumpy:     ['흥…', '말 걸지 마', '삐졌어'],
        welcome:    ['어디 갔었어!', '오랜만이야', '기다렸어'],
        bondUp:     ['우리 좀 친해진 것 같아', '너 좋아', '같이 있으니까 좋다']
      }
    },
    {
      id: 'b', name: '친구', on: false, size: 82, speed: 1.0,
      ...
      lines: {
        ...
        sleep:      ['쿨쿨…', '잘 자…'],
        wake:       ['음…?', '벌써 아침?'],
        foodSpotted:['밥 발견!', '내가 먼저!'],
        eat:        ['냠', '맛있어'],
        full:       ['잘 먹었어!', '배부르다'],
        favorite:   ['이거 진짜 좋아해!', '최고야'],
        hateFood:   ['우웩…', '어쩔 수 없이'],
        foodLost:   ['에이…', '다음엔 내가 먹을 거야'],
        greet:      ['어 왔어?', '같이 놀자'],
        reply:      ['좋아', '응 그래'],
        fightStart: ['내가 먼저 왔거든?', '덤벼'],
        fightTaunt: ['저리 가!', '내 거야!'],
        fightWin:   ['이겼다 이겼다!', '내 거야~'],
        fightLose:  ['아 진짜…', '삐졌어'],
        makeUp:     ['이제 안 싸울래', '미안했어'],
        happy:      ['기분 좋다!', '룰루'],
        grumpy:     ['…', '심심해 죽겠어'],
        welcome:    ['왔구나!', '보고 싶었어'],
        bondUp:     ['우리 친구지?', '헤헤 좋아']
  ...
  Mascot.prototype.findFood = function () {
    ...
    if (PET.on && this.hunger > 95) return null;
      ...
      if (this.eating) {
        ...
        if (f) {
          ...
          if (Math.abs(dx) < S * .45 && f.landed) this.startEat(f);
            ...
            this.dir = dx > 0 ? 1 : -1;
            var runIt = this.isHungry() || this.pref(f.type.id) === 2 || Math.abs(dx) > 300;
  ...
  function initSticker() {
    ...
    var iv2 = setInterval(function () {
      var right = document.querySelector('#taskbar .taskbar-right') || document.getElementById('taskbar');
      if (!right) { if (++tries > 40) clearInterval(iv2); return; }
      clearInterval(iv2);
      if (document.getElementById('smj-lock')) return;
      var b = document.createElement('button');
      b.type = 'button';
      b.id = 'smj-lock';
      b.className = 'taskbar-button';
      b.addEventListener('click', function () { setLock(!lockOn); });
      var quick = right.querySelector('.taskbar-quick');
      if (quick) quick.appendChild(b); else right.insertBefore(b, right.firstChild);
      lockBtn = b;
      setLock(lockOn);
    }, 250);
  }
