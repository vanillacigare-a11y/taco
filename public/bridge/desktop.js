...
(function () {
  ...
  function snapshot() {
    ...
    return { keys: keys, icons: pos, theme: themeTokens(), win: winBoxes() };
  }
  ...
  function stickerSpot() {
    ...
    return {
      x: Math.max(8, Math.round(window.innerWidth / 2 - 80) + step),
      y: Math.max(8, Math.round((window.innerHeight - barH) / 2 - 80) + step)
    };
  }
