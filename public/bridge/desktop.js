...
(function () {
  ...
  function gateStyle() {
    ...
    s.textContent = [
      ...
      'html.dds-quiet-slot .slot-gallery{visibility:hidden}'
    ].join('\n');
    document.head.appendChild(s);
  }
  ...
  function shapeBlog(tries) {
    ...
    /* 5:3 로, 화면을 꽉 채우지 않게 — 가로의 62% 쯤.
       (넓이로 치면 화면 꽉 채웠을 때의 대략 1/3 이다) */
    var bar = document.getElementById('taskbar');
    ...
    var w = Math.round(window.innerWidth * 0.62);
    w = Math.max(560, Math.min(w, roomW));       /* 너무 작아도 못 쓰니 아래로 한계를 둔다 */
