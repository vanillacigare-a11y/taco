...
(function () {
  ...
  /* 바탕화면은 화면(슬롯)이 가진 것이다 — 슬롯 글 안에 wallpaper 로 들어간다.
     그런데 이 값을 여기 보따리에도 같이 담아 모두에게 뿌리고 있었다. 그러면
     마지막에 저장한 바탕화면 하나가 모든 화면을 덮어써서,
     「겨울 바탕화면이 여름에 가 있고, 화면을 갈아입으면 또 사라지는」 일이 났다.
     그래서 슬롯이 하나라도 있으면 여기서는 뺀다 (슬롯이 없으면 담을 데가
     없으니 예전처럼 여기에 담는다). */
  var WALL_KEYS = /^(wallpaper|wallpaperPost|wallpaperFit)$/;
  var slotCount = -1;                 /* -1 = 아직 안 세어봄 */

  function skip(k) {
    /* 「지금 무엇을 보고 있나」 류는 사람마다 달라야 한다.
       이런 게 서버에 저장되면 모든 방문자가 한 상태에 묶인다 (layoutSlot 사고). */
    if (/Slot$|^current|^last|^view$|^active/.test(k)) return true;
    if (WALL_KEYS.test(k) && slotCount > 0) return true;
    return k === 'booted' ||
      k === 'layoutSlot' ||
      k.indexOf('gallery:') === 0 ||
  ...
  /* ── 자리 잠그기 ─────────────────────────────────────────────────────
     자물쇠 버튼은 마스코트 쪽(shimeji.js)에 이미 있다. 그게 스티커를 잠그는데,
     같은 버튼으로 바탕화면 아이콘까지 잠근다. 잠겼는지는 html 에 붙는
     표시(smj-sticker-locked)만 보면 되니, 마스코트가 없어도 안 깨진다.

     잠금은 화면(슬롯)마다 따로 기억한다 — 여름은 잠그고 겨울은 풀어둘 수 있게.
     기억은 서버에 저장한다 (다른 기기에서 봐도 똑같게). */
  var locks = {};
  try {
    if (window.DDS_LOCKS && typeof window.DDS_LOCKS === 'object') locks = window.DDS_LOCKS;
  } catch (e) { locks = {}; }

  function isLocked() {
    try { return document.documentElement.classList.contains('smj-sticker-locked'); } catch (e) { return false; }
  }

  /* 이 화면에 저장해둔 잠금 상태를 자물쇠에 입힌다.
     quiet=true 로 불러서, 「우리가 입힌 것」이 다시 저장거리로 잡히지 않게 한다. */
  function applyLock() {
    var want = !!locks[wearingNow()];
    try {
      if (window.SMJ && window.SMJ.setLock) window.SMJ.setLock(want, true);
      else document.documentElement.classList.toggle('smj-sticker-locked', want);
    } catch (e) {}
  }

  /* 사람이 자물쇠를 눌렀을 때 — 지금 입고 있는 화면에 적어두고 저장거리로 표시 */
  document.addEventListener('smj:lock', function (e) {
    var on = !!(e && e.detail && e.detail.on);
    var who = wearingNow();
    if (on) locks[who] = true; else delete locks[who];
    try { markDirty(); } catch (err) {}
  });
  ...
  /* 자리 없는 아이콘에게 빈 격자 칸을 하나씩 준다.
     - 어떤 칸이 차 있는지는 「자리를 정해둔 아이콘들」로만 센다
     - 나눠주는 순서는 이름순 (DOM 순서를 쓰면 다시 그릴 때마다 달라진다)
     - 정해준 자리는 pos 에 적어둬서 다음에 또 계산하지 않는다 */
  function assignFreeSpots(host, list, hb) {
    var need = [];
    for (var i = 0; i < list.length; i++) {
      var k = iconKey(list[i]);
      if (k && !pos[k]) need.push({ key: k, el: list[i] });
    }
    if (!need.length) return;
    need.sort(function (a, b) { return a.key < b.key ? -1 : a.key > b.key ? 1 : 0; });

    var one = list[0] ? list[0].getBoundingClientRect() : null;
    var cw = Math.round((one && one.width) || 88) + GAP;
    var ch = Math.round((one && one.height) || 96) + GAP;
    var rows = Math.max(1, Math.floor(((hb.height || 600) - 16) / ch));

    /* 이미 누가 앉아 있는 칸 */
    var taken = {};
    for (var k2 in pos) {
      if (!pos.hasOwnProperty(k2)) continue;
      var col = Math.round((pos[k2][0] - 12) / cw);
      var row = Math.round((pos[k2][1] - 16) / ch);
      taken[col + ':' + row] = 1;
    }

    for (var n = 0, cell = 0; n < need.length; n++) {
      while (taken[Math.floor(cell / rows) + ':' + (cell % rows)] && cell < 400) cell++;
      var c = Math.floor(cell / rows), r = cell % rows;
      taken[c + ':' + r] = 1;
      pos[need[n].key] = [12 + c * cw, 16 + r * ch];
      cell++;
    }
  }
  ...
  function applyPositions() {
    ...
    /* 자리를 정해둔 적 없는 아이콘에게 자리를 하나 잡아준다.
       예전에는 그 자리를 「지금 DOM 에서 몇 번째냐」로 계산했다(12 + i*4).
       그런데 이 함수는 아이콘 목록이 바뀔 때마다 다시 도는데다, 스킨이 아이콘을
       다시 그리면 순서가 달라진다. 그래서 자리 없는 아이콘이 새로 그릴 때마다
       4px 씩 기어다녔다 (28,32 → 24,28 → 20,24 …).

       이제는 빈 격자 칸을 찾아 넣고 그 자리를 기억한다.
       순서가 아니라 이름순으로 정하니 다시 그려도 같은 자리에 앉는다. */
    assignFreeSpots(host, list, hb);
  ...
  function snapshot() {
    ...
    return { keys: keys, icons: pos, theme: themeTokens(), win: winBoxes(), locks: locks };
  }

  /* 열려 있는 창들의 자리·크기.

     스킨은 「위젯」 창만 슬롯에 담는다 (dw() 가 widget:/embed: 키만 고른다).
     블로그·프로필·갤러리·휴지통 창은 아무도 안 챙겨줘서, 저장해도 다음에
     들어오면 없거나 기본 크기로 돌아갔다 — 그걸 우리가 담는다. */
  function ourWin(w) {
    var key = w.dataset ? (w.dataset.winKey || '') : '';
    if (!key) return '';
    if (key.indexOf('widget:') === 0 || key.indexOf('embed:') === 0) return '';  /* 슬롯 담당 */
    var t = ((w.querySelector('.win-title') || {}).textContent || '').trim();
    if (/^(슬롯|꾸미기 저장|사진 제목)/.test(t)) return '';   /* 우리가 잠깐 여는 창 */
    return key;
  }
    ...
    try {
      ...
      for (var i = 0; i < wins.length; i++) {
        ...
        out[key] = {
          ...
          t: ((w.querySelector('.win-title') || {}).textContent || '').trim().slice(0, 40)
        };
      }
    } catch (e) {}
    return out;
  ...
  function savedWins() {
    try { return window.DDS_WIN || {}; } catch (e) { return {}; }
  }

  /* 창이 화면에 나타나는 그 순간 크기를 맞춘다.
     나중에 맞추면 작게 떴다가 「드득」 하고 커지는 게 보인다. */
  function fitWin(w) {
    if (!w || w.dataset.ddsBox) return;
    var key = ourWin(w);
    if (!key) return;
    w.dataset.ddsBox = '1';

    var box = savedWins()[key];
    if (!box && key === 'post-browser') box = defaultBlogBox();
    if (!box || !(box.w > 40)) return;
    ...
    var maxW = Math.max(320, window.innerWidth - 8);
    var maxH = Math.max(240, window.innerHeight - barH - 8);
    var bw = Math.min(box.w, maxW), bh = Math.min(box.h, maxH);

    w.style.width = bw + 'px';
    w.style.height = bh + 'px';
    /* style 로 준 값과 그려진 크기가 테두리만큼 어긋난다 — 한 번 재서 맞춘다 */
    var r = w.getBoundingClientRect();
    if (r.width && r.height) {
      w.style.width = (bw - (r.width - bw)) + 'px';
      w.style.height = (bh - (r.height - bh)) + 'px';
      r = w.getBoundingClientRect();
    }
    var x = box.x, y = box.y;
    if (x == null || y == null) {
      x = Math.round((window.innerWidth - r.width) / 2);
      y = Math.round((window.innerHeight - barH - r.height) / 2);
    }
    w.style.left = Math.max(0, Math.min(x, window.innerWidth - r.width)) + 'px';
    w.style.top = Math.max(0, Math.min(y, window.innerHeight - barH - r.height)) + 'px';
  }

  /* 저장해둔 게 하나도 없을 때 블로그 창이 뜨는 크기 — 5:3, 화면 가로의 34% */
  function defaultBlogBox() {
    var barH = 40;
    try {
      var bar = document.getElementById('taskbar');
      if (bar) barH = bar.getBoundingClientRect().height;
    ...
    var w = Math.max(420, Math.min(Math.round(window.innerWidth * 0.34), roomW));
    var h = Math.round(w * 3 / 5);
    if (h > roomH) { h = roomH; w = Math.round(h * 5 / 3); }
    return { w: w, h: h, x: null, y: null };
  }

  /* 스킨은 위젯 창을 늘 일반 창 아래 깔아둔다 (위젯 100·101, 창 102·103 …).
     그래서 프로필·갤러리를 띄워두면 새로 연 위젯이 그 뒤에 숨어버린다.
     방금 연 창은 맨 앞으로 올려준다. */
  var frontReady = false;
  setTimeout(function () { frontReady = true; }, 4000);   /* 들어올 때 되살리는 건 건드리지 않는다 */

  function toFront(w) {
    if (!frontReady || !w || w.dataset.ddsFront) return;
    w.dataset.ddsFront = '1';
    setTimeout(function () {
      try {
        var top = 0;
        var wins = document.querySelectorAll('.win');
        for (var i = 0; i < wins.length; i++) {
          ...
          var z = parseInt(getComputedStyle(wins[i]).zIndex, 10);
          if (z > top) top = z;
        }
        if (top && parseInt(getComputedStyle(w).zIndex, 10) <= top) {
          w.style.zIndex = String(top + 1);
        }
      } catch (e) {}
    }, 60);
  }

  /* 스킨은 뭔가 바뀔 때마다 위젯을 다시 아래로 내린다.
     그래서 「누른 창이 앞으로」를 우리가 계속 지켜준다 — 창다운 동작이기도 하고. */
  function initFront() {
    document.addEventListener('pointerdown', function (e) {
      var w = e.target.closest ? e.target.closest('.win') : null;
      if (!w) return;
      delete w.dataset.ddsFront;
      frontReady = true;
      toFront(w);
    }, true);
  }
  ...
  function initWinLayout() {
    var scan = function () {
      var wins = document.querySelectorAll('.win');
      for (var i = 0; i < wins.length; i++) fitWin(wins[i]);
    };
    scan();
    try {
      new MutationObserver(function (ms) {
        for (var i = 0; i < ms.length; i++) {
          ...
          for (var j = 0; j < add.length; j++) {
            ...
            if (n.classList && n.classList.contains('win')) { fitWin(n); toFront(n); }
            else if (n.querySelectorAll) {
              var inner = n.querySelectorAll('.win');
              for (var k = 0; k < inner.length; k++) { fitWin(inner[k]); toFront(inner[k]); }
            }
          }
        }
  ...
  /* 저장해둔 창이 다 뜰 때까지 화면을 가려둔다.
     스킨의 로딩 화면은 한 세션에 한 번만 떠서, 새로고침하면 그게 없다.
     그러면 빈 바탕화면이 0.3초쯤 보였다가 창이 툭툭 뜬다 — 그걸 덮는다. */
  function veilUp() {
    if (document.getElementById('dds-veil')) return;
    var n = 0;
    try { n = Object.keys(savedWins()).length; } catch (e) {}
    if (!n) return;                        /* 되살릴 게 없으면 가릴 이유도 없다 */

    var bg = '#dcdcd8';
    try {
      var c = (window.DDS_CONFIG && window.DDS_CONFIG.loadingBg) || '';
      if (/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(c)) bg = c;
    } catch (e) {}

    var v = document.createElement('div');
    v.id = 'dds-veil';
    v.style.cssText = 'position:fixed;inset:0;z-index:99998;background:' + bg + ';' +
      'transition:opacity .18s ease;pointer-events:none';
    (document.body || document.documentElement).appendChild(v);

    /* 무슨 일이 있어도 3초 뒤에는 걷힌다 */
    setTimeout(veilDown, 3000);
  }

  function veilDown() {
    var v = document.getElementById('dds-veil');
    if (!v) return;
    v.style.opacity = '0';
    setTimeout(function () { if (v.parentNode) v.parentNode.removeChild(v); }, 220);
  }

  /* 저장해둔 창이 다 「제자리에 앉을 때까지」 지켜보다가 가림막을 걷는다.
     예전엔 창이 뜬 개수만 셌다. 그런데 창은 스킨이 정한 자리에 먼저 뜨고,
     저장해둔 자리로 옮기는 건 fitWin 이 그 다음에 한다.
     그래서 가림막이 먼저 걷히고 나서 창들이 우르르 미끄러져 들어갔다
     (매번 28px 씩 — «저장할 때 창이 이동된다» 가 이거였다).
     fitWin 이 자리를 잡으면 창에 ddsBox 표시를 남기니, 그걸 기다린다. */
  function veilWatch() {
    var want = 0;
    try { want = Object.keys(savedWins()).length; } catch (e) {}
    if (!want) { veilDown(); return; }
    var end = Date.now() + 2800;
    var last = '';
    var still = 0;
    (function look() {
      var n = 0, placed = 0, sig = [];
      var wins = document.querySelectorAll('.win');
      for (var i = 0; i < wins.length; i++) {
        var w = wins[i];
        var r = w.getBoundingClientRect();
        sig.push(Math.round(r.left) + ',' + Math.round(r.top));   /* 위젯까지 전부 본다 */
        if (!ourWin(w)) continue;
        n++;
        if (w.dataset && w.dataset.ddsBox) placed++;
      }
      /* 자리가 두 번 연속 그대로여야 「멈췄다」로 본다.
         스킨이 뜨면서 창 전체를 한 번 더 밀기 때문에, 그게 끝나기 전에 걷으면
         창들이 우르르 미끄러지는 게 그대로 보인다. */
      var now = sig.join('|');
      still = (now === last) ? still + 1 : 0;
      last = now;

      if ((n >= want && placed >= want && still >= 2) || Date.now() > end) {
        setTimeout(veilDown, 60);
        return;
      }
      setTimeout(look, 60);
    })();
  ...
  /* 저장해둔 창들을 다시 띄운다 (바탕화면 아이콘을 대신 눌러서).
     저장해둔 게 하나도 없으면 예전처럼 블로그 창만 띄운다. */
  function restoreWins(tries) {
    var saved = savedWins();
    var keys = [];
    for (var k in saved) if (Object.prototype.hasOwnProperty.call(saved, k)) keys.push(k);
    if (!keys.length) { veilDown(); autoOpenBlog(0); return; }

    /* 아이콘이 생기자마자 바로 연다.
       스킨의 로딩 화면이 걷히기 전에 끝나야, 빈 바탕화면이 잠깐 보이지 않는다.
       (아이콘 850ms → 로딩 걷힘 1550ms 쯤이라 그 사이에 들어간다)
       슬롯은 위젯 창만 다루니 우리 창과 겹칠 일도 없다 — 기다릴 이유가 없다. */
    var host = document.getElementById('desktop-icons');
    if (!host || !host.querySelector('.desktop-icon')) {
      if ((tries || 0) > 120) return;
      setTimeout(function () { restoreWins((tries || 0) + 1); }, 80);
      return;
    }
    veilWatch();
    var open = {};
    var wins = document.querySelectorAll('.win');
    for (var i = 0; i < wins.length; i++) {
      var kk = wins[i].dataset ? wins[i].dataset.winKey : '';
      if (kk) open[kk] = 1;
    }
    var icons = host.querySelectorAll('.desktop-icon');
    keys.forEach(function (key, n) {
      if (open[key]) return;
      var want = (saved[key] && saved[key].t) || '';
      if (!want) return;
      for (var i = 0; i < icons.length; i++) {
        var l = icons[i].querySelector('.desktop-icon-label');
        if (l && l.textContent.trim() === want) {
          /* 자리는 곧바로 fitWin 이 잡아주니 거의 동시에 열어도 된다 */
          (function (el) { setTimeout(function () { el.click(); }, n * 40); })(icons[i]);
          return;
        }
      }
    });
  ...
  /* 관리 화면이 같은 색을 쓰도록  /* 관리 화면이 같은 색을 쓰도록, 지금 화면에 실제로 적용된 색을 그대로 담아 보낸다 */
  var THEME_KEYS = [
  ...
  /* ── 꾸미기 저장, 빠른 길 ──────────────────────────────────────────────
     스킨에는 「지금 화면을 통째로 찍는」 기능도, 「슬롯 글을 만드는」 기능도
     이미 들어 있다. 다만 밖에서 부를 수 없게 닫혀 있었을 뿐이라,
     우리는 여태 슬롯 창을 열고 편집 모드에 들어가는 순서를 대신 클릭해줬다.
     느리고(1.5초), 창이 깜빡이고, 끝에 새로고침까지 필요했다.

     이제 서버가 스킨을 내보낼 때 그 기능들을 꺼내 쓸 수 있게 해준다
     (src/skinassets.js). 그러면 저장이 이걸로 끝난다:

       지금 화면을 찍고 → 슬롯 글에 갈아끼우고 → 서버에 보낸다

     창도 편집 모드도 새로고침도 없다. 만들어지는 글은 스킨이 자기 기능으로
     만든 것이라, 사람이 손으로 저장한 것과 완전히 같다.

     창구가 없으면(스킨이 바뀌었으면) false 를 돌려주고, 예전 방식으로 돌아간다. */
  function saveDecoFast(done) {
    var api = window.__ddsSlot;
    if (!api || typeof api.snapshot !== 'function') { done(false); return; }

    var layout = null;
    try { layout = api.snapshot(); } catch (e) { layout = null; }
    if (!layout || typeof layout !== 'object') { done(false); return; }

    var give = function () { done(false); };

    fetch('/api/slot', { credentials: 'same-origin' })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (j) {
        var text = (j && j.text) || '';
        if (!text.trim()) throw new Error('빈 슬롯');
        return Promise.resolve(api.parse(text)).then(function (slots) {
          if (!slots || !slots.length) throw new Error('슬롯 없음');
          slotCount = slots.length;

          /* 어느 화면에 담을까 — 지금 입고 있는 것.
             이 브라우저가 갈아입은 게 있으면 그것, 없으면 글에 표시된 것,
             그것도 없으면 첫 번째. 엉뚱한 화면에 덮어쓰지 않게 이름을 꼭 맞춘다. */
          var names = slots.map(function (s) { return s.name; });
          var want = '';
          try { want = sessionStorage.getItem(P + 'layoutSlot') || ''; } catch (e) {}
          try { want = JSON.parse(want); } catch (e) {}
          if (names.indexOf(want) === -1) { try { want = api.active(text) || ''; } catch (e) { want = ''; } }
          if (names.indexOf(want) === -1 && typeof window.DDS_SLOT === 'string') want = window.DDS_SLOT;
          if (names.indexOf(want) === -1) want = names[0];

          var next = slots.map(function (s) {
            return s.name === want ? { name: s.name, layout: layout } : s;
          });
          return api.build(next, want);
        });
      })
      .then(function (text) {
        if (!text || !String(text).trim()) throw new Error('만들어진 글이 비었어');
        return fetch('/api/slot', {
          ...
          return r.json().then(function (j) { if (!r.ok) throw new Error(j.message || '실패'); });
        });
      })
      .then(function () {
        /* 스킨한테도 「지금이 저장된 상태」라고 알려준다 —
           안 그러면 나갈 때 「저장 안 한 게 있다」고 붙잡는다 */
        try { api.seen(); } catch (e) {}
  ...
  function save() {
    ...
    /* 보내기 전에 화면 목록을 한 번 읽는다.
       - 바탕화면을 어디에 담을지 정하고 (슬롯이 있나 없나)
       - 배경이 슬롯에 담긴 것과 다르면 슬롯까지 저장하도록 표시한다 */
    countSlots().then(sendDesktop, sendDesktop);
  }
  ...
  function sendDesktop() {
    fetch('/api/desktop', {
      ...
      .then(function () {
        ...
        /* 편집 모드라면 스킨 자신의 저장을 눌러야 한다.
           「＋ 화면 만들기 · 지우기」로 새로 만든 화면은 이 저장에만 담긴다.
           우리 빠른 길은 서버에 이미 있는 화면 목록만 고쳐서, 새 화면을 통째로
           떨어뜨린다 (만들어도 사라지던 이유). */
        if (inEditMode() && document.querySelector('.dds-editbar-apply')) {
          note('새 화면까지 저장하는 중…');
          clearDeco();
          try { document.querySelector('.dds-editbar-apply').click(); } catch (e) {}
          return;
        }
        ...
        if (decoDirty) {
          /* 빠른 길이 되면 그걸로 (창도 새로고침도 없다).
             안 되면 예전처럼 편집 모드를 대신 눌러준다. */
          saveDecoFast(function (ok) {
            if (ok) { clearDeco(); note('저장했어. 이제 손님 화면에도 이렇게 보여'); return; }
            note('화면까지 저장하는 중…');
  ...
  /* 바탕화면을 바꾸면 스킨이 화면을 다시 불러온다. 그때 「아직 슬롯에 안 담았다」는
     기억이 날아가서, 저장을 눌러도 바탕화면이 슬롯에 안 들어갔다. 그래서 적어둔다. */
  var DECO_KEY = 'bridge:decoDirty';
  var decoDirty = false;
  try { decoDirty = sessionStorage.getItem(DECO_KEY) === '1'; } catch (e) {}

  function markDeco() {
    decoDirty = true;
    try { sessionStorage.setItem(DECO_KEY, '1'); } catch (e) {}
    markDirty();
  }
  function clearDeco() {
    decoDirty = false;
    try { sessionStorage.removeItem(DECO_KEY); } catch (e) {}
  }
  ...
  function decoSig() {
    var out = [];
    try {
      /* 바탕화면도 슬롯이 가진 것이다. 여기서 안 세면 배경을 바꿔도
         슬롯에는 안 담기고, 갈아입거나 새로 들어오면 사라진다. */
      var wk = ['wallpaper', 'wallpaperPost', 'wallpaperFit', 'dither'];
      for (var n = 0; n < wk.length; n++) {
        var v = null;
        try { v = sessionStorage.getItem(P + wk[n]); } catch (e) { v = null; }
        out.push('bg|' + wk[n] + '=' + (v == null ? '' : String(v).slice(0, 200)));
      }
      ...
      /* 위젯 창만 본다. 블로그·프로필·갤러리 창은 우리가 따로 저장하니까
         여기서 세면 창을 조금만 옮겨도 슬롯 저장이 통째로 돌아간다. */
      var wins = document.querySelectorAll('.win');
      for (var k = 0; k < wins.length; k++) {
        var w = wins[k];
        var key = w.dataset ? (w.dataset.winKey || '') : '';
        if (key.indexOf('widget:') !== 0) continue;
        out.push('w|' + key + ',' + w.style.left + ',' + w.style.top +
  ...
  /* 우리가 따로 저장하는 창들(블로그·프로필·갤러리·휴지통)의 지문.
     이쪽이 바뀌면 저장 버튼만 켜면 된다 — 슬롯까지 갈 일이 없다. */
  function winSig() {
    var out = [];
    try {
      var wins = document.querySelectorAll('.win');
      for (var i = 0; i < wins.length; i++) {
        ...
        if (!ourWin(w)) continue;
        var r = w.getBoundingClientRect();
        out.push(w.dataset.winKey + '|' + Math.round(r.left) + ',' + Math.round(r.top) +
  ...
  function watchDeco() {
    ...
    setTimeout(function () {
      ...
      setInterval(function () {
        if (inEditMode()) {
          /* 편집 모드에선 스킨이 꾸미기를 챙긴다. 다만 화면을 새로 만들거나 지우면
             그건 저장을 눌러야 서버에 담기니, 저장 버튼만 켜준다. */
          var now = [];
          var cs = document.querySelectorAll('.dds-card[data-slot-name]');
          for (var c = 0; c < cs.length; c++) now.push(cs[c].getAttribute('data-slot-name'));
          now = now.sort().join('|');
          if (cardBase === null) cardBase = now;
          else if (now !== cardBase) { cardBase = now; markDirty(); }
          return;
        }
        cardBase = null;

        var w = winSig();
        if (w !== winBase) { winBase = w; markDirty(); }   /* 창만 바뀜 → 슬롯은 안 건드림 */

        var now = decoSig();
        if (now === decoBase) return;
        decoBase = now;
        markDeco();                         /* 스티커·위젯·바탕화면이 바뀜 → 슬롯까지 저장해야 함 */
      }, 1200);
    }, 3000);
  }
  ...
  /* 우리가 열어둔 슬롯 창을 닫는다.
     사람이 직접 열어둔 게 아니라 저장하려고 몰래 연 것이라, 실패해도 남아 있으면 안 된다. */
  function closeSlotWin() {
    try {
      var g = document.querySelector('.slot-gallery');
      var w = g && g.closest ? g.closest('.win') : null;
      var x = w && w.querySelector('.win-close');
      if (x) x.click();
    } catch (e) {}
  }
  ...
  function saveDecoration(giveUp) {
    ...
    startHush();
    /* 중간에 어그러져도 열어둔 슬롯 창은 반드시 닫고 나간다.
       예전엔 여기서 창을 안 닫아서, 저장이 실패할 때마다 슬롯 창이 맨 앞에 남았다.
       («저장하고 나면 슬롯 편집창이 위에 뜬다» 의 나머지 절반이 이거였다.) */
    var stop = function (told, keepOpen) {
      if (!keepOpen) closeSlotWin();
      endEditMode();
      liftQuiet();
      giveUp(told);
    };
    ...
    waitFor('.slot-gallery', 5000, function (g) {
      ...
      /* 슬롯이 하나도 없으면 담을 데가 없다.
         이때만 창을 열어둔다 — 사람이 여기서 바로 만들어야 하니까. */
      ...
      if (!edit) { stop(); return; }
      edit.click();
      waitFor('.dds-editbar-apply', 6000, function (b) {
        /* 슬롯 창은 닫고 저장한다 — 열어둔 채로 담으면 그 창까지 화면에 저장되고,
           다른 창들 자리도 그때 모습으로 밀려서 저장된다 */
        var g2 = document.querySelector('.slot-gallery');
        var w2 = g2 && g2.closest ? g2.closest('.win') : null;
        var x2 = w2 && w2.querySelector('.win-close');
        if (x2) x2.click();
        setTimeout(function () { b.click(); }, 260);
      }, stop, false);
    }, stop);
  }
  ...
  function initDrag() {
    ...
    host.addEventListener('pointerdown', function (e) {
      ...
      if (isLocked()) return;      /* 자물쇠가 잠겨 있으면 끌리지 않는다 (누르면 열리는 건 그대로) */
  ...
  /* ── 「라벨 스티커」 창을 「사진 스티커」 창으로 ────────────────────────
     스킨의 라벨 스티커는 글자만 붙일 수 있고, 사진을 붙이려면 먼저 갤러리에
     등록해야 했다. 갤러리에 남기고 싶지 않은 사진까지 다 들어가서 번거롭다.

     라벨은 안 쓰니 이 창을 사진 스티커 창으로 바꾼다. 여기서 올린 사진은
     재료로만 저장되고 갤러리 목록에는 안 나온다.

     스티커는 스킨의 함수로 만든다(창구 addSticker). 그래야 끌기·크기·색·효과·
     지우기가 전부 스킨 것과 똑같이 동작한다. 우리가 직접 만들어 넣으면 굳어버린다. */
  /* 새 스티커를 붙일 자리.
     한가운데에 두면 창 밑에 깔려서 보이지도, 잡히지도 않는다.
     창이 안 덮은 빈 자리를 찾아서 거기 붙인다 (없으면 오른쪽 아래). */
  function stickerSpot() {
    ...
    var W = window.innerWidth, H = window.innerHeight - barH;
    var size = 160, pad = 24;
    var n = document.querySelectorAll('.sticker-global').length;
    var slide = (n % 5) * 28;                       /* 여러 장이면 조금씩 어긋나게 */

    function clear(x, y) {
      try {
        var pts = [[x + size / 2, y + size / 2], [x + 10, y + 10], [x + size - 10, y + size - 10]];
        for (var i = 0; i < pts.length; i++) {
          var el = document.elementFromPoint(pts[i][0], pts[i][1]);
          if (!el) return false;
          if (el.closest && el.closest('.win')) return false;      /* 창에 가림 */
          if (el.closest && el.closest('#taskbar')) return false;
        }
        return true;
      } catch (e) { return true; }
    }

    /* 오른쪽 위 → 오른쪽 아래 → 왼쪽 아래 순으로 빈 자리를 본다 */
    var tries = [];
    for (var col = 0; col < 4; col++) {
      for (var row = 0; row < 4; row++) {
        tries.push([
          W - pad - size - col * (size + pad) + slide,
          pad + row * (size + pad) + slide
        ]);
      }
    }
    for (var t = 0; t < tries.length; t++) {
      var x = Math.max(8, Math.min(tries[t][0], W - size - 8));
      var y = Math.max(8, Math.min(tries[t][1], H - size - 8));
      if (clear(x, y)) return { x: Math.round(x), y: Math.round(y) };
    }
    return {
      x: Math.max(8, Math.round(W - size - pad)),
      y: Math.max(8, Math.round(H - size - pad))
    };
  }

  function photoStickerPanel(panel) {
    if (!panel || panel.dataset.ddsPhoto) return;
    panel.dataset.ddsPhoto = '1';

    var api = window.__ddsSlot;
    if (!api || typeof api.addSticker !== 'function') return;   /* 창구가 없으면 그냥 둔다 */

    /* 라벨 쪽 안내·목록·버튼은 CSS 로 가린다.
       hidden 을 붙이면 스킨이 다시 그릴 때 되살아난다. */
    panel.classList.add('dds-photo-panel');

    var title = panel.querySelector('.dds-section-title');
    if (title) title.textContent = '사진 스티커';
    var win = panel.closest ? panel.closest('.win') : null;
    var wt = win && win.querySelector('.win-title');
    if (wt) wt.textContent = '사진 스티커';
    var tab = win && win.dataset.winKey
      ? document.querySelector('#taskbar .taskbar-tab[title="' + '라벨 스티커 관리' + '"]') : null;
    if (tab) { tab.textContent = '사진 스티커'; tab.title = '사진 스티커'; }

    var b = document.createElement('button');
    b.type = 'button';
    b.className = 'dds-btn dds-photo-sticker';
    b.textContent = '＋ 사진으로 스티커 만들기';

    var guide = document.createElement('p');
    guide.className = 'dds-guide';
    guide.textContent = '컴퓨터에서 고른 사진이 바탕화면에 스티커로 붙어. ' +
      '갤러리에는 안 들어가. 크기·색·모션·지우기는 바탕화면의 스티커를 우클릭해서 바꿔. ' +
      '다 붙였으면 작업표시줄의 「저장」을 눌러줘.';
    ...
    b.addEventListener('click', function () {
      pickFile(function (f) {
        if (f && Array.isArray(f)) f = f[0];
        if (!f) return;
        var old = b.textContent;
        b.disabled = true; b.textContent = '올리는 중…';
        var fd = new FormData();
        fd.append('file', f);
        /* 재료로 올린다 — 갤러리 목록에는 안 남는다 */
        post('/api/assets', fd)
          .then(function (j) {
            if (!j.path) throw new Error('주소를 못 받았어');
            var at = stickerSpot();
            var ok = api.addSticker({ src: j.path, x: at.x, y: at.y, width: 160 });
            b.disabled = false; b.textContent = old;
            if (!ok) { note('스티커를 못 붙였어'); return; }
            markDirty();
            note('붙였어. 자리를 잡고 「저장」을 눌러줘');
          })
          .catch(function (e) {
            b.disabled = false; b.textContent = old;
            note('올리기 실패 — ' + e.message);
          });
      });
    });
  }
  ...
  function watchWindows() {
    var scan = function (node) {
      ...
      onEach(node, '.label-manager', photoStickerPanel);
      slotWordsAround();
    };
    scan(document.body);
    new MutationObserver(function (ms) {
  ...
  /* 가림막(몰래 작업하는 동안 화면을 덮어두는 것)을 걷는다 */
  function liftQuiet() {
    try {
      stopHush();
  ...
  /* ── 저장이 끝난 뒤 뒷정리 ──────────────────────────────────────────────
     예전에는 저장이 끝나면 페이지를 통째로 다시 불러왔다.
     스킨이 「코드를 붙여넣고 → 새로고침」 하는 흐름으로 만들어져 있어서,
     새로고침을 해야 스킨이 들고 있던 편집 상태가 정리됐기 때문이다.

     그런데 그 새로고침이 대가가 컸다. 창이 전부 닫혔다 다시 열리면서
     제자리를 찾아 우르르 미끄러지고 («저장할 때 창이 이동된다»),
     그 사이 어긋난 화면이 보이지 않게 가림막을 오래 씌워야 했고,
     저장 한 번에 1.5초가 걸렸다.

     새로고침 없이도 정리할 수 있다는 걸 확인했다. 우리가 열어놓은 것을
     우리가 닫으면 된다: 「게시글에 반영」 창을 닫고, 편집 모드를 끝낸다.
     그러면 창도 스티커도 하나도 안 움직이고 0.3초에 끝난다.

     혹시 편집 모드가 안 꺼지면 그때만 예전처럼 새로고침으로 물러난다. */
  function closeReflectWin() {
    try {
      var r = document.querySelector('.slot-reflect');
      var w = r && r.closest ? r.closest('.win') : null;
      var x = w && w.querySelector('.win-close');
  ...
  function endEditMode() {
    try {
      var bar = document.querySelector('.dds-editbar');
      if (!bar) return true;                       /* 이미 꺼져 있음 */
      var bs = bar.querySelectorAll('button');
      for (var i = 0; i < bs.length; i++) {
        /* 「꾸미기 저장」(dds-editbar-apply) 이 아니라 「편집 끝내기」 를 눌러야 한다.
           둘을 헷갈려서 여태 편집 모드가 안 꺼지고 있었다. */
        if (bs[i].classList.contains('dds-editbar-apply')) continue;
        if (/편집\s*끝내기|끝내기|나가기/.test(bs[i].textContent || '')) { bs[i].click(); return true; }
      }
    } catch (e) {}
    return false;
  }
  ...
  function finishQuietly() {
    ...
    /* 편집 모드가 정말 꺼졌는지 잠깐 지켜본다.
       꺼졌으면 가림막만 걷고 끝. 안 꺼지면 예전 방식(새로고침)으로 물러난다. */
    var end = Date.now() + 1500;
    (function look() {
      if (!document.querySelector('.dds-editbar')) {
        closeReflectWin();
        setTimeout(liftQuiet, 60);
        return;
      }
      if (Date.now() > end) {
        try { location.reload(); } catch (e) { liftQuiet(); }
        return;
      }
      endEditMode();
      setTimeout(look, 120);
  ...
  function slotReflect(box) {
    ...
    fetch('/api/slot', {
      ...
      .then(function (r) { return r.json().then(function (j) { if (!r.ok) throw new Error(j.message || '실패'); return j; }); })
      .then(function () {
        say.textContent = '저장했어.';
        note('꾸미기를 저장했어');

        /* 스킨이 들고 있는 임시본은 지운다 — 안 지우면 나중에 되살아난다 */
        try { localStorage.removeItem('dds:slot-draft'); } catch (e) {}
  ...
  /* 글자 한 줄 물어보는 작은 창 (사진 제목 창과 같은 모양을 쓴다) */
  function askText(opt, done) {
    ...
    bar.querySelector('.win-title').textContent = opt.title || '';
    ...
    ok.type = 'button'; ok.className = 'dds-btn dds-tt-ok';
    ...
    function key(e) {
      ...
      else if (e.key === 'Enter') { e.preventDefault(); close(inp.value.trim()); }
    }
    cancel.addEventListener('click', function () { close(null); });
    ok.addEventListener('click', function () { close(inp.value.trim()); });
  ...
  /* 카드마다 작은 「이름」 단추를 붙인다 */
  function renameButtons(g) {
    var cards = g.querySelectorAll('.dds-card[data-slot-name]');
    for (var i = 0; i < cards.length; i++) {
      (function (card) {
        if (card.querySelector('.dds-slot-rename')) return;
        var name = card.getAttribute('data-slot-name') || '';
        if (!name) return;

        var b = document.createElement('button');
        b.type = 'button';
        b.className = 'dds-btn dds-slot-rename';
        b.textContent = '이름';
        b.title = '「' + name + '」 이름 바꾸기';
        /* 카드를 누르면 그 화면으로 갈아입는다 — 이 단추는 거기까지 안 가게 막는다 */
  ...
  function askRename(from) {
    askText({
      title: '화면 이름 바꾸기',
      hint: '「' + from + '」 의 새 이름을 적어줘. 이 화면에 올린 사진들도 같이 따라가.',
      value: from,
      placeholder: '예: 봄',
      ok: '바꾸기',
      max: 40
      ...
      if (to === null) return;
      if (!to) { note('이름이 비었어'); return; }
      if (to === from) return;

      fetch('/api/slot/rename', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        ...
        .then(function (r) {
          return r.json().then(function (j) { if (!r.ok) throw new Error(j.message || '실패'); return j; });
        })
        .then(function () {
          /* 이름으로 기억하던 것들을 이 브라우저에서도 옮긴다 */
          if (locks[from]) { delete locks[from]; locks[to] = true; }
          try {
            var w = JSON.parse(sessionStorage.getItem(P + 'layoutSlot') || '""');
            if (w === from) sessionStorage.setItem(P + 'layoutSlot', JSON.stringify(to));
          } catch (e) {}
          try {
            if (localStorage.getItem(SLOT_PICK) === from) localStorage.setItem(SLOT_PICK, to);
          } catch (e) {}
          markWearing();
          note('「' + from + '」 → 「' + to + '」 로 바꿨어');
          setTimeout(function () { location.reload(); }, 700);
        })
        .catch(function (e) { note('이름을 못 바꿨어 — ' + e.message); });
    });
  }
  ...
  function slotShape(g) {
    ...
    /* 카드마다 「이름 바꾸기」 — 스킨에는 이름을 고치는 길이 아예 없어서,
       이름이 맘에 안 들면 화면을 새로 만드는 수밖에 없었다. */
    if (isOwner()) renameButtons(g);
  ...
  /* ── 화면(슬롯)마다 다른 갤러리 ──────────────────────────────────────
     스킨은 갤러리 사진을 「갤러리 저장소」 글에서 읽어간다. 그 글은 서버가 내주니,
     지금 입고 있는 화면을 쿠키로 알려주면 서버가 그 화면 사진만 남겨서 준다.
     (서버 쪽은 render.js 의 galleryBodyFor 참고) */
  function wearingNow() {
    var v = '';
    try { v = sessionStorage.getItem(P + 'layoutSlot') || ''; } catch (e) {}
    try { v = JSON.parse(v); } catch (e) {}
    if (typeof v === 'string' && v) return v;
    try { return wantSlot() || ''; } catch (e) { return ''; }
  }

  /* 화면(슬롯)이 몇 개인지 세어둔다 — 바탕화면을 어디에 담을지 정하는 데 쓴다.
     (슬롯이 있으면 슬롯에, 하나도 없으면 예전처럼 브라우저 값 보따리에) */
  /* 화면에 떠 있는 배경과 지금 입은 화면에 담긴 배경이 다르면,
     저장할 때 슬롯까지 가야 한다. 배경을 바꾸면 스킨이 화면을 다시 불러오는데,
     그 사이에 「바뀌었다」는 기억이 흐려질 때가 있어서 여기서 확실히 짚는다. */
  function wallDrift(list) {
    try {
      if (!list || !list.length) return;
      var who = wearingNow();
      var mine = null;
      for (var i = 0; i < list.length; i++) if (list[i].name === who) { mine = list[i]; break; }
      if (!mine) mine = list[0];
      var saved = (mine && mine.layout && mine.layout.wallpaper) || '';
      var now = '';
      try { now = JSON.parse(sessionStorage.getItem(P + 'wallpaper') || '""') || ''; } catch (e) { now = ''; }
      if (String(saved) !== String(now)) markDeco();
    } catch (e) {}
  }

  function countSlots() {
    try {
      return fetch('/api/slot', { credentials: 'same-origin' })
        .then(function (r) { return r.json(); })
        .then(function (j) { return window.__ddsSlot.parse(j.text || ''); })
        .then(function (list) {
          if (list && list.length != null) slotCount = list.length;
          wallDrift(list);
          return slotCount;
        })
        .catch(function () { return slotCount; });
    } catch (e) { return Promise.resolve(slotCount); }
  }

  function markWearing(name) {
    var v = String(name == null ? wearingNow() : name);
    try {
      document.cookie = 'dds_slot=' + encodeURIComponent(v) +
  ...
  /* 화면을 갈아입었으면 갤러리도 다시 읽게 한다 (사진 목록이 달라지니까) */
  function regallery() {
    clearGalleryCache();
    var gs = document.querySelectorAll('.gallery');
    for (var i = 0; i < gs.length; i++) refreshGallery(gs[i]);
  }


  /* 슬롯 창 손질은 손님한테도 해야 한다 —
     주인 도구(watchWindows) 안에만 두면 손님은 스킨 원래 문구를 그대로 본다. */
  var slotAskedAt = 0;              /* 사람이 슬롯 버튼을 누른 시각 */
  function initSlotWindow() {
    /* 사람이 슬롯 창을 열려고 누른 건지 기록해 둔다 */
    document.addEventListener('click', function (e) {
      var b = e.target.closest ? e.target.closest('button') : null;
      if (!b) return;
      var t = (b.getAttribute('title') || '') + ' ' + (b.textContent || '');
      if (/슬롯/.test(t)) slotAskedAt = Date.now();
    }, true);
  ...
  function keepPick(name) {
    /* 갤러리는 화면마다 다르다 — 쿠키를 고치고 목록을 다시 읽는다 */
    markWearing(name);
    setTimeout(regallery, 300);
    if (isOwner()) {
    ...
    if (g) {
      ...
      for (var i = 0; i < cards.length; i++) {
        if (cards[i].dataset.slotName === want) {
          ...
          /* 갤러리는 화면마다 다르니 쿠키도 같이 맞춘다 */
          markWearing(want);
          setTimeout(regallery, 400);
      ...
      pickFile(function (f) {
        ...
        post('/api/wallpaper', fd)
          .then(function (j) {
            ...
            /* 바탕화면은 슬롯에 담겨야 한다. 바로 아래에서 화면을 다시 불러오니
               기억이 날아가지 않게 적어두는 markDeco 를 쓴다. */
            markDeco();
  ...
  function askTitles(files, done) {
    ...
    var head = document.createElement('p');
    head.className = 'dds-tt-head';
    /* 어느 화면의 갤러리로 들어가는지 여기서 알려준다 —
       안 그러면 올리고 나서 다른 화면에서 안 보일 때 영문을 모른다 */
    var into = '';
    try { into = wearingNow(); } catch (e) {}
    head.textContent = into
      ? '「' + into + '」 화면의 갤러리에 올라가. 다른 화면에서는 안 보여.'
      : '모든 화면의 갤러리에 올라가.';
    var head2 = document.createElement('p');
    head2.className = 'dds-tt-head';
    head2.style.margin = '2px 0 6px';
    head2.textContent = '제목은 크게 봤을 때 사진 아래에 뜨는 이름이야. 비워둬도 돼.';
    body.appendChild(head);
  ...
  /* 사진 제목 고치기 — 지금 화면에 떠 있는 사진들만 다룬다.
     어느 화면의 갤러리인지는 올릴 때 정해지고(그때 입고 있던 화면), 여기서는 안 건드린다. */
  function editTitles(g) {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      var path = '';
      try { path = new URL(im.src, location.href).pathname.replace(/\?.*$/, ''); } catch (e) { continue; }
  ...
  function galleryPanel(g) {
    ...
    b.addEventListener('click', function () {
      pickFile(function (files) {
        /* pickFile 은 「여러 장」일 때만 배열을 준다. 고르러 들어온 창은 한 장 모드라
           파일 하나가 그냥 오는데, 아래에서 files[0] 을 꺼내 쓰고 있었다.
           파일 하나에서 [0] 을 꺼내면 undefined 가 되어 서버에 빈 것이 갔고,
           「파일이 안 왔어」로 끝났다 — 쉐이커 배경·토큰, 투두·캘린더 이미지,
           배경화면 고르기까지 「고르러 들어온 창의 올리기」가 전부 이것 때문에 안 됐다.
           여기서 모양을 한 번 맞춰두면 아래는 그대로 써도 된다. */
        ...
        askTitles(files, function (titles) {
          ...
          /* 지금 입고 있는 화면의 갤러리에 넣는다 (화면마다 사진이 다르다) */
          try { fd.append('slot', wearingNow()); } catch (e) {}
          post('/api/uploads', fd)
            .then(function (j) {
  ...
  function galleryDelete(g, row) {
    ...
    doIt.addEventListener('click', function () {
      ...
      var left = list.length, failed = 0, why = '';
      list.forEach(function (p) {
        /* 화면의 주소는 한글이 %EB%B4%84… 로 바뀌어 있을 수 있다.
           서버는 둘 다 알아듣지만, 되도록 원래 글자로 보내준다. */
        var send = p;
        try { send = decodeURIComponent(p); } catch (e) { send = p; }
        fetch('/api/uploads/remove', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ path: send, raw: p }),
          credentials: 'same-origin'
        })
          .then(function (r) {
            if (r.ok) return;
            failed++;
            return r.json().then(function (j) { if (!why) why = j.message || ''; }, function () {});
          })
          .catch(function (e) { failed++; if (!why) why = e.message; })
          .then(function () {
            if (--left > 0) return;
            on = false; picked = {}; paint();
            clearGalleryCache();
            refreshGallery(g);
            /* 왜 못 지웠는지 알려준다 — 예전엔 「못 지웠어」 만 떠서 손쓸 데가 없었다 */
            if (failed) note(failed + '장은 못 지웠어' + (why ? ' — ' + why : ''));
          });
      });
    });
  ...
  function injectStyle() {
    ...
    s.textContent = [
      ...
      /* 화면 카드 위의 작은 「이름」 단추 */
      '.dds-slot-rename{position:absolute;right:4px;top:4px;z-index:3;',
        'padding:1px 6px;font-size:11px;line-height:1.5;opacity:.8}',
      '.dds-slot-rename:hover{opacity:1}'
    ].join('\n');
    document.head.appendChild(s);
  }
  ...
  function gateStyle() {
    ...
    s.textContent = [
      ...
      /* 「게시글에 반영」 창(.slot-reflect)도 같이 가린다.
         예전엔 슬롯 창만 가려서, 우리가 대신 눌러주는 동안 이 창이 맨 앞에 그대로 떴다. */
      'html.dds-quiet-slot .win:has(.slot-gallery),html.dds-quiet-slot .win:has(.slot-reflect),',
      'html.dds-quiet-slot .dds-quiet-win{',
      '  position:fixed!important;left:-99999px!important;top:0!important;',
      '  opacity:0!important;pointer-events:none!important}',
      '.dds-quiet-tab{display:none!important}',
      /* 라벨 스티커 창을 사진 스티커 창으로 쓴다 — 라벨 쪽 조각은 감춘다 */
      '.dds-photo-panel .label-manager-empty,.dds-photo-panel .label-row-list,',
      '.dds-photo-panel .label-add-btn{display:none!important}',
      '.dds-photo-sticker{margin-top:4px}'
    ].join('\n');
    document.head.appendChild(s);
  }
  ...
  /* ── 사진 주소 고쳐 끼우기 ────────────────────────────────────────────
     스킨은 꾸미기에 쓴 사진 주소를 「절대 주소」로 저장한다
     (http://지금주소/uploads/…). 그리고 그 주소가 죽었을 때를 대비해
     슬롯 글에 붙어 있는 사진 목록에서 살아 있는 주소를 찾아 바꿔 끼우는
     장치를 갖고 있다 — 티스토리 사진은 시간이 지나면 주소가 죽으니까.

     우리 사진은 안 죽는다. 그래서 그 목록은 아무 일도 안 하면서
     사진 수만큼 길어지기만 했다 (페이지 열 때마다 읽는 글인데도).
     목록은 걷어내고, 그 일을 여기서 대신한다:
     화면에 뜬 사진 중 「우리 서버의 /uploads/ 인데 주소가 지금과 다른 것」을
     지금 주소로 고쳐 끼운다. 나중에 도메인이 바뀌어도 안 깨진다. */
  function fixOne(raw) {
    if (!raw || raw.indexOf('//') === -1) return '';          /* 이미 상대 주소 */
    var u;
    try { u = new URL(raw, location.href); } catch (e) { return ''; }
    if (u.origin === location.origin) return '';               /* 지금 주소면 그대로 */
    if (u.pathname.indexOf('/uploads/') !== 0) return '';      /* 우리 사진만 */
  ...
  function fixUploadUrls() {
    ...
    for (var i = 0; i < imgs.length; i++) {
      var f = fixOne(imgs[i].getAttribute('src'));
      if (f) imgs[i].setAttribute('src', f);
    }
    /* 배경으로 깔린 사진 (배경화면·아이콘·위젯 등) */
    var els = document.querySelectorAll('[style*="/uploads/"]');
    for (var k = 0; k < els.length; k++) {
      var st = els[k].getAttribute('style') || '';
      if (st.indexOf('//') === -1) continue;
      var next = st.replace(/url\((['"]?)([^'")]+)\1\)/g, function (all, q, addr) {
        var g = fixOne(addr);
        return g ? 'url(' + q + g + q + ')' : all;
      });
      if (next !== st) els[k].setAttribute('style', next);
    }
  }

  /* ── 갤러리 격자에 작은 사진 쓰기 ──────────────────────────────────────
     티스토리에서는 다음 CDN 이 사진을 알아서 줄여줬다. 스킨에 그 코드가 아직 있지만
     카카오·다음·티스토리 주소에만 동작해서 우리 사진은 해당이 없다.
     그래서 격자 한 칸이 120px 인데 2000px 원본을 그대로 받고 있었다.

     서버에 ?w=400 을 붙이면 줄여서 준다. 그런데 이 <img> 의 src 는 격자에만 쓰이는 게
     아니다 — 사진을 끌어다 스티커로 붙이거나 배경화면으로 정할 때 스킨이 바로 이 src 를
     읽어간다. 작은 걸 물고 가면 스티커·배경이 흐려진다.

     그래서 「보고만 있을 땐 작은 것, 손대는 순간 원본」으로 한다.
     누르기·끌기 둘 다 pointerdown 이 먼저 일어나므로, 거기서 원본으로 되돌리면
     스킨이 읽을 때는 이미 원본이다. 되돌린 건 다시 줄이지 않는다. */
  var SMALL_W = 400;

  function smallSrc(raw) {
    if (!raw) return '';
    var u;
    try { u = new URL(raw, location.href); } catch (e) { return ''; }
    if (u.origin !== location.origin) return '';
    if (u.pathname.indexOf('/uploads/') !== 0) return '';
    if (u.search) return '';                                   /* 이미 뭔가 붙어 있음 */
    return u.pathname + '?w=' + SMALL_W;
  }

  /* watch() 가 MutationObserver 로도 부르기 때문에 인자를 받지 않는다 */
  function shrinkGrid() {
    var imgs = document.querySelectorAll('.gallery-grid img[src]');
    for (var i = 0; i < imgs.length; i++) {
      var im = imgs[i];
      if (im.dataset.ddsFull || im.dataset.ddsKeep) continue;  /* 이미 처리했거나 손댄 것 */
      var raw = im.getAttribute('src');
      var s = smallSrc(raw);
      if (!s) continue;
      im.dataset.ddsFull = raw;
      im.setAttribute('src', s);
    }
  }

  function unshrink(el) {
    var im = el && el.closest ? el.closest('.gallery-card, .gallery-grid') : null;
    if (!im) return;
    var list = im.tagName === 'IMG' ? [im] : im.querySelectorAll('img[data-dds-full]');
    for (var i = 0; i < list.length; i++) {
      var f = list[i].dataset.ddsFull;
      if (!f) continue;
      list[i].setAttribute('src', f);
      delete list[i].dataset.ddsFull;
      /* 다시 줄이지 않게 표시해둔다.
         (표시를 안 남기면 다음 화면 변화 때 shrinkGrid 가 도로 줄여버린다) */
      list[i].dataset.ddsKeep = '1';
    }
  }

  function initGridThumbs() {
    watch('갤러리 작은 사진', shrinkGrid);
    /* 캡처 단계 — 스킨이 src 를 읽기 전에 우리가 먼저 되돌린다 */
    document.addEventListener('pointerdown', function (e) {
      try { unshrink(e.target); } catch (err) {}
    }, true);
  ...
  function initUploadUrls() {
    watch('사진 주소', fixUploadUrls);
    initGridThumbs();
  }

  /* ── 디데이 배너 ────────────────────────────────────────────────────
     스킨은 디데이 칸을 <a href="글주소"> 로 만들어서, 누르면 그 글이 창으로 열린다
     (「…:: 블로그.exe」). 디데이는 날짜를 보는 것이지 글로 갈 데가 아니라서 막는다.
     고를 때 쓰는 딱지(dday-banner-pick)는 그대로 둔다. */
  function initDday() {
    document.addEventListener('click', function (e) {
      var a = e.target.closest ? e.target.closest('.dday-banner') : null;
      if (!a) return;
      if (a.classList.contains('dday-banner-pick')) return;   /* 고르는 화면은 그대로 */
      e.preventDefault();
      e.stopPropagation();
    }, true);
  }
  ...
  function autoOpenBlog(tries) {
    ...
    for (var i = 0; i < icons.length; i++) {
      ...
      if (l && (l.textContent || '').trim() === '블로그') {
        icons[i].click();          /* 크기는 창이 뜨는 순간 fitWin 이 맞춘다 */
        return;
      }
    }
  ...
  function start() {
    ...
    safe('입고 있는 화면 표시', function () { markWearing(); })();
    safe('화면 개수 세기', countSlots)();
    /* 자물쇠 버튼은 마스코트가 늦게 뜬다 — 뜨고 나서 한 번 더 맞춰준다 */
    setTimeout(function () { try { applyLock(); } catch (e) {} }, 1200);
    setTimeout(function () { try { applyLock(); } catch (e) {} }, 3000);
    safe('가림막', veilUp)();
