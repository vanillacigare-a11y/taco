...
(function () {
  ...
  function veilWatch() {
    ...
    (function look() {
      var n = 0;
      var wins = document.querySelectorAll('.win');
      for (var i = 0; i < wins.length; i++) if (ourWin(wins[i])) n++;
      if (n >= want || Date.now() > end) { setTimeout(veilDown, 80); return; }
      setTimeout(look, 60);
    })();
  }
  ...
  function slotReflect(box) {
    ...
    fetch('/api/slot', {
      ...
      .then(function () {
        ...
        /* 새로고침이 막히더라도 숨김 상태가 남지 않게 */
        try {
          stopHush();
          document.documentElement.classList.remove('dds-quiet-slot');
          var hid0 = document.querySelectorAll('.dds-quiet-win,.dds-quiet-tab');
          for (var q = 0; q < hid0.length; q++) {
            hid0[q].classList.remove('dds-quiet-win');
            hid0[q].classList.remove('dds-quiet-tab');
          }
        } catch (e) {}
