...
(function () {
  ...
  function galleryPanel(g) {
    ...
    if (picking) { tools.hidden = false; more.classList.add('is-open'); }
  ...
  function refreshBoard(done) {
    fetch('/api/board/state', { credentials: 'same-origin' })
      ...
      .then(function (j) {
        ...
        /* 스킨한테 지금 보던 화면을 다시 불러오라고 시킨다 */
        var here = document.querySelector('.cat-tree-list .is-active, .cat-tree-list [aria-current]') ||
                   document.querySelector('.cat-tree-list a, .cat-tree-list button');
        if (here) here.click();
        setTimeout(function () { dressList(); if (done) done(); }, 700);
      })
      .catch(function () {
  ...
  function addFeatureMenu() {
    ...
    for (var i = 0; i < arts.length; i++) {
      ...
      var items = [
        ['수정', function () { menu.hidden = true; openPostEditor(id, art); }],
        ...
        ['비공개로 전환', function () {
          ...
          }).then(function () { refreshBoard(); note('공개 상태를 바꿨어'); });
        }],
        ['삭제', function () {
          menu.hidden = true;
          if (!window.confirm('이 글을 지울까? 되돌릴 수 없어.')) return;
          fetch('/api/posts/' + id, { method: 'DELETE', credentials: 'same-origin' })
            .then(function () { refreshBoard(); note('글을 지웠어'); });
        }]
      ];
  ...
  function initWrite() {
    watch('글쓰기 버튼', function () { addWriteButton(); addFeatureMenu(); });
  }
  ...
  /* ── 블로그 창 자동으로 열기 ─────────────────────────────────────────── */
  /* 들어오면 블로그 창이 떠 있게 한다. 새로고침해도 마찬가지.
     이미 열려 있는 창이 있으면 끼어들지 않는다 (우리 로그인 창은 창으로 안 친다). */
  function autoOpenBlog(tries) {
    if (document.querySelector('.win:not(.dds-gate-win)')) return;
    ...
    for (var i = 0; i < icons.length; i++) {
      ...
      if (l && (l.textContent || '').trim() === '블로그') { icons[i].click(); return; }
    }
