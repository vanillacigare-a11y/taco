...
(function () {
  ...
  function iconUploadBtn(galleryBtn) {
    ...
    b.addEventListener('click', function () {
      pickFile(function (f) {
        ...
        post('/api/uploads', fd)
          .then(function (j) {
            var url = j.items && j.items[0] && j.items[0].path;
  ...
  /* 갤러리 창 — 이미지 올리기 (고르는 중일 때는 넣지 않는다) */
  function galleryPanel(g) {
    if (g.querySelector('.dds-gal-upload')) return;
    if (g.querySelector('.gallery-pick-banner')) return;
    ...
    hint.textContent = '여러 장 한 번에 올려도 돼';
    row.append(b, hint);
    ...
    b.addEventListener('click', function () {
      pickFile(function (files) {
        ...
        post('/api/uploads', fd)
          .then(function (j) {
            /* 스킨이 갤러리 목록을 5분간 기억하고 있어서, 그 기억을 지워야 새로 읽는다 */
            try {
              var del = [];
              for (var i = 0; i < sessionStorage.length; i++) {
                var k = sessionStorage.key(i);
            ...
            b.textContent = ((j.items || []).length) + '장 올렸어. 새로고침…';
    ...
    var banner = g.firstChild;
    if (banner) g.insertBefore(row, banner); else g.appendChild(row);
  }
  ...
  function injectStyle() {
    ...
    s.textContent = [
      ...
      '.dds-panel-save{display:flex;align-items:center;gap:8px;flex-wrap:wrap;' +
        'margin-top:var(--dds-space-sm,10px);padding-top:var(--dds-space-sm,10px);' +
