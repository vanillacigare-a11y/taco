...
(function () {
      ...
      new MutationObserver(function (ms) {
        for (var i = 0; i < ms.length; i++) {
          ...
          for (var j = 0; j < add.length; j++) {
            ...
            if (n.classList && n.classList.contains('win')) fitWin(n);
            else if (n.querySelectorAll) {
              var inner = n.querySelectorAll('.win');
              for (var k = 0; k < inner.length; k++) fitWin(inner[k]);
            }
          }
        }
  ...
  function initSlotWindow() {
    ...
    /* 아무도 안 눌렀는데 슬롯 창이 떴다 = 우리(또는 스킨)가 뒤에서 연 것.
       무슨 경로로 열렸든 눈에는 안 띄게 치운다. */
    watch('슬롯 창 감추기', function () {
      var g = document.querySelector('.slot-gallery');
      if (!g) return;
      var byHand = Date.now() - slotAskedAt < 4000;
      if (byHand) return;
      if (!document.documentElement.classList.contains('dds-quiet-slot')) {
        document.documentElement.classList.add('dds-quiet-slot');
        startHush();
        /* 뒤에서 하는 일이 끝날 시간을 주고 원래대로 */
        setTimeout(unquiet, 2500);
      }
    });
