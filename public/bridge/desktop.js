...
(function () {
  ...
  function watchWindows() {
    var scan = function (node) {
      ...
      onEach(node, '.dds-editbar-apply', function (el) {
        ...
        el.textContent = '저장';
        el.title = '누르면 바로 저장돼 — 손님 화면에도 반영돼';
      });
  ...
  function slotReflect(box) {
    ...
    fetch('/api/slot', {
      ...
      .then(function (r) { return r.json().then(function (j) { if (!r.ok) throw new Error(j.message || '실패'); return j; }); })
      .then(function () {
        say.textContent = '저장했어. 손님 화면에도 이대로 보여.';
        note('꾸미기를 저장했어');
      })
      .catch(function (e) {
  ...
  function slotWords(root) {
    ...
    for (var i = 0; i < hits.length; i++) {
      ...
      if (!/게시글|본문|슬롯은 2023|보기와 전환만/.test(t)) continue;
  ...
  function captionStyle() {
    ...
    s.textContent = [
      ...
      '  font-size:12.5px;line-height:1.4;opacity:.9;pointer-events:none}',
      '.dds-cap span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%}',
  ...
  function shapeBlog(tries) {
    ...
    /* 화면에 들어가는 선에서 제일 큰 5:3 */
    var pad = 48;
    var bar = document.getElementById('taskbar');
    var barH = bar ? bar.getBoundingClientRect().height : 40;
    var maxW = Math.max(320, window.innerWidth - pad * 2);
    var maxH = Math.max(240, window.innerHeight - barH - pad * 2);

    var w = Math.min(maxW, Math.round(maxH * 5 / 3));
    var h = Math.round(w * 3 / 5);
    if (h > maxH) { h = maxH; w = Math.round(h * 5 / 3); }
