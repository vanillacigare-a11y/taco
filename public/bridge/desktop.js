      ...
      pickFile(function (files) {
        ...
        post('/api/uploads', fd)
          .then(function (j) {
            ...
            refreshGallery(g);
            if (!picking) {
              b.textContent = items.length + '장 올렸어';
              setTimeout(function () { b.disabled = false; b.textContent = '＋ 컴퓨터에서 올리기'; }, 1600);
              return;
            }
            /* 고르러 들어온 거니까, 방금 올린 걸 대신 눌러준다 (갤러리를 거칠 필요가 없어진다) */
            b.textContent = '적용하는 중…';
            waitForCard(g, items[0] && items[0].path, function (card) {
              b.disabled = false; b.textContent = '＋ 컴퓨터에서 올리기';
              if (card) card.click();
              else note('올리긴 했는데 목록에서 못 찾았어. 목록에서 직접 눌러줘');
            });
          })
  ...
  function directBtn(galleryBtn, label) {
    ...
    b.addEventListener('click', function () {
      pickFile(function (f) {
        ...
        post('/api/uploads', fd)
          .then(function (j) {
            var path = j.items && j.items[0] && j.items[0].path;
            clearGalleryCache();
            galleryBtn.click();
            waitForGallery(function (g) {
              if (!g) { b.disabled = false; b.textContent = old; note('올리긴 했는데 적용을 못 했어. 「갤러리에서 선택」에서 골라줘'); return; }
              refreshGallery(g);
              waitForCard(g, path, function (card) {
                b.disabled = false; b.textContent = old;
                if (card) card.click();
                else note('올리긴 했는데 목록에서 못 찾았어. 「갤러리에서 선택」에서 골라줘');
              });
            });
          })
