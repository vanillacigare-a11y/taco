...
(function () {
  ...
  /* 블로그 창 크기·자리.
     스킨은 「위젯」 창만 슬롯에 담는다 (dw() 가 위젯 키만 고른다).
     블로그 창은 위젯이 아니라서 아무도 안 챙겨준다 — 우리가 담는다. */
  function winBoxes() {
    var out = {};
    try {
      var w = document.querySelector('.win-post-browser');
      if (w) {
        var r = w.getBoundingClientRect();
        if (r.width > 40 && r.height > 40) {
          out.blog = { w: Math.round(r.width), h: Math.round(r.height),
  ...
  function autoOpenBlog(tries) {
    /* 저장해둔 화면에 블로그 창이 들어 있으면 스킨이 알아서 띄운다.
       그게 깔릴 시간을 주고 나서 판단해야, 우리가 새로 열어 크기를 덮어쓰지 않는다. */
    if (!tries && !autoOpenBlog.waited) {
      autoOpenBlog.waited = 1;
      setTimeout(function () { autoOpenBlog(0); }, 2600);
      return;
    }
  ...
  function shapeBlog(tries) {
    ...
    /* 저장해둔 크기가 있으면 그대로 되살린다 (기본 크기로 되돌리지 않는다) */
    var saved = null;
    try { saved = (window.DDS_WIN && window.DDS_WIN.blog) || null; } catch (e) {}
    if (saved && saved.w > 40 && saved.h > 40) {
      win.style.width = saved.w + 'px';
      win.style.height = saved.h + 'px';
      if (saved.x != null) win.style.left = Math.max(0, saved.x) + 'px';
      if (saved.y != null) win.style.top = Math.max(0, saved.y) + 'px';
      var rr = win.getBoundingClientRect();
      win.style.width = (saved.w - (rr.width - saved.w)) + 'px';
      win.style.height = (saved.h - (rr.height - saved.h)) + 'px';
      return;
    }
