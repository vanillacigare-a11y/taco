...
(function () {
  ...
  function openMenu(m, x, y) {
    ...
    if (STICKER.lock && canEditStickers()) {
      items.push(null);
      items.push([lockOn ? '잠금 풀기 (스티커·아이콘)' : '잠그기 (스티커·아이콘)',
        function () { setLock(!lockOn); }]);
    }
    items.push(null);
  ...
  /* 이 자물쇠는 스티커뿐 아니라 바탕화면 아이콘도 같이 잠근다.
     - 스티커는 아래 initSticker 의 캡처 단계 차단으로 막는다
     - 아이콘은 desktop.js 가 html 의 smj-sticker-locked 를 보고 안 끌리게 한다
     - 잠갔는지 여부는 화면(슬롯)마다 다르다. 그 기억은 desktop.js 가 서버에 맡기고,
       여기서는 smj:lock 사건을 알리기만 한다 (quiet=true 면 되돌려 알리지 않는다) */
  function setLock(on, quiet) {
    ...
    if (lockBtn) {
      ...
      lockBtn.innerHTML = lockIcon(lockOn) +
        '<span class="smj-lock-label">' + (lockOn ? '잠김' : '잠그기') + '</span>';
      lockBtn.title = lockOn
        ? '스티커와 바탕화면 아이콘이 잠겼어 — 누르면 다시 옮길 수 있어'
        : '스티커와 바탕화면 아이콘을 지금 자리에 잠근다';
    ...
    if (quiet) return;
    try {
      document.dispatchEvent(new CustomEvent('smj:lock', { detail: { on: lockOn } }));
    } catch (e) {}
  }
  function initSticker() {
    if (!STICKER.lock) return;
    /* 서버에 저장해둔 화면별 값이 오기 전까지 쓸 임시값 (그 뒤 desktop.js 가 덮어쓴다) */
    ...
    if (!canEditStickers()) { setLock(lockOn, true); return; }
  ...
  window.SMJ = {
    ...
    setLock: function (v, quiet) { setLock(v, quiet); }, isLocked: function () { return lockOn; },
