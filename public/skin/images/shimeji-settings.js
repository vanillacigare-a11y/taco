...
(function () {
  ...
  function moveTab() {
    return sw('G|auto', '자율 행동', S.G.auto) +
      ...
      '</div></div>' +
      '<div class="smjs-sec open"><h4><span>잠</span><span></span></h4><div>' +
      rng('sleepAfter', '가만히 두면 잠드는 시간',
          Math.round(S.G.sleepAfter / 1000), 10, 600, 10, sleepLabel(S.G.sleepAfter)) +
      '<div class="smjs-hint">건드리면 바로 깨. 자는 동안엔 위아래로 숨만 쉬어.</div>' +
      '</div></div>' +
      ...
      '</div></div>' +
      '<div class="smjs-sec open"><h4><span>자리 잠금</span><span></span></h4><div>' +
      sw('lock', '스티커·바탕화면 아이콘 잠금 (실수로 안 밀리게)', S.isLocked()) +
  ...
  function applyRange(key, v, el) {
    ...
    else if (key === 'sleepAfter') { S.G.sleepAfter = v * 1000; disp = sleepLabel(v * 1000); }
