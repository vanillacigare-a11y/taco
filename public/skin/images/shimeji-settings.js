...
(function () {
  ...
  function saveTab() {
    ...
    return '<div class="smjs-row"><label>지금 설정 크기</label><b>' + kb(size) + '</b></div>' +
      ...
      '<button class="smjs-b on" data-act="store">이 브라우저에 저장</button>' +
      '<button class="smjs-b" data-act="forget">저장한 것 지우기</button>' +
      '<button class="smjs-b" data-act="export">설정 코드 만들기</button>' +
      '</div>' +
      '<div class="smjs-hint"><b>이 브라우저에 저장</b>은 네 화면에서만 적용돼 — 미리 맞춰보는 용도야.<br>' +
      '방문자에게도 똑같이 보이게 하려면 <b>설정 코드 만들기</b>로 나온 내용을 <code>shimeji.js</code> 의 ' +
      '<code>■ 설정 시작 ~ ■ 설정 끝</code> 사이와 바꿔서 다시 업로드해야 해.</div>' +
  ...
  function doAction(b) {
    ...
    else if (p[0] === 'store') { storeConfig(); b.textContent = '저장됨!'; setTimeout(function () { b.textContent = '이 브라우저에 저장'; }, 1400); }
    else if (p[0] === 'forget') {
      try { localStorage.removeItem('smj:config'); } catch (e) {}
      b.textContent = '지웠어'; setTimeout(function () { b.textContent = '저장한 것 지우기'; }, 1400);
    }
    else if (p[0] === 'export') {
  ...
  function storeConfig() {
    try { localStorage.setItem('smj:config', JSON.stringify(snapshot())); } catch (e) { alert('저장 실패: ' + e.message); }
  }
