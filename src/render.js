...
function template() {
  ...
  /* jQuery 는 티스토리 기본 스킨이 늘 넣어주던 것이라 이 스킨도 따라 넣었을 뿐,
     스킨 어디서도 쓰지 않는다 (script.js 의 $ 는 압축기가 만든 자기 함수다).
     안 쓰는 87KB 를 방문자마다 바깥 서버에서 받게 할 이유가 없어서 뺀다. */
  s = s.replace(/[ \t]*<script src="https:\/\/code\.jquery\.com\/[^"]*"><\/script>\n?/g, '');

  /* 폰트·아이콘 주소를 우리 서버 것으로 (skinassets.js 설명 참고) */
  s = skinassets.swap(s, 'skin.html');
...
/* 목록 줄의 사진은 손톱만 하게만 뜨고 크게 볼 일이 없다 — 줄인 걸 보낸다 */
function listItemHtml(p) {
  const thumb = p.thumb ? `<img class="t-thumb" src="${esc(thumbs.small(p.thumb, 400))}" alt="" loading="lazy">` : '';
...
/* ── 화면(슬롯)마다 다른 갤러리 ──────────────────────────────────────────
   스킨은 갤러리 사진을 「갤러리 저장소」 글의 본문에서 읽어간다.
   그 본문에는 올린 사진이 전부 들어 있어서, 어느 화면을 입고 있든 같은 사진이 떴다.

   사진마다 「어느 화면의 갤러리인가」를 적어두고(uploads.slot),
   지금 입고 있는 화면이 아닌 사진은 여기서 걸러낸다.
   빈 값인 사진은 모든 화면에 뜬다 (예전에 올린 사진이 사라지지 않게).

   지금 입고 있는 화면은 브라우저가 쿠키로 알려준다 — 화면을 갈아입으면
   bridge/desktop.js 가 쿠키를 고치고 갤러리를 다시 읽게 한다. */
function galleryBodyFor(html, wearing) {
  const want = String(wearing || '');
  let map;
  try { map = uploads.slotMap(); } catch (e) { return html; }
  if (!map) return html;
  return String(html).replace(/<img\b[^>]*>/gi, (tag) => {
    const m = /\ssrc=["']([^"']+)["']/i.exec(tag);
    if (!m) return tag;
    const own = map[m[1]];
    if (!own) return tag;                 /* 모든 화면에 뜨는 사진 */
    return own === want ? tag : '';       /* 다른 화면의 사진이면 뺀다 */
  });
}

function articleHtml(p, tags, owner, wearing) {
  if (p.system_kind === 'gallery') {
    p = { ...p, body_html: galleryBodyFor(p.body_html, wearing) };
  }
...
function dataBlock(view) {
  ...
  if (view.kind === 'list') {
    ...
    parts.push(articleHtml(view.post, view.tags, view.owner, view.wearing));
...
function desktopSeed() {
  ...
  /* 「지금 어느 화면을 입고 있나」는 사람마다 다르다 — 옛날에 저장된 게 남아 있으면
     그게 모든 방문자를 한 화면에 묶어버린다. 뿌리지 않는다. */
  delete keys.layoutSlot;
  const icons = (st && st.icons) || {};
  const win = (st && st.win) || {};
  const locks = (st && st.locks) || {};
  if (!Object.keys(keys).length && !Object.keys(icons).length && !Object.keys(locks).length) return '';
  return '<script>(function(){var K=' + jsonInScript(keys) + ',P=' + jsonInScript(icons) + ';' +
    ...
    'window.DDS_ICON_POS=P;window.DDS_WIN=' + jsonInScript(win) + ';' +
    'window.DDS_LOCKS=' + jsonInScript(locks) + ';})();</script>\n';
}
...
function postMetaSeed() {
  ...
  for (const p of rows) {
    ...
    out['/' + p.id] = {
      ...
      t: thumbs.small(p.thumb || firstImage(p.body_html) || '', 400),
      c: p.category || '',
      n: p.is_notice ? 1 : 0
    };
  }
...
/* 주인이 골라둔 바탕화면 이름 — 손님이 처음 볼 화면 */
function slotPickSeed() {
  let name = '';
  try { name = settings.get('board:slot', '') || ''; } catch (e) { return ''; }
  if (!name) return '';
  return '<script>window.DDS_SLOT=' + jsonInScript(name) + ';</script>\n';
}

/* 주인이 화면을 한 번이라도 저장했나 — 그랬으면 창 크기를 우리가 건드리지 않는다 */
function winSavedSeed() {
  let v = '';
  try { v = settings.get('board:winSaved', '') || ''; } catch (e) { return ''; }
  if (!v) return '';
  return '<script>window.DDS_WIN_SAVED=1;</script>\n';
}
...
/* ── 스킨이 「데이터만」 가져갈 때 ───────────────────────────────────────────
   스킨은 다른 목록·글이 필요하면 그 페이지를 통째로 다시 받아서 #tistory-data 만
   꺼내 쓴다. 티스토리엔 API 가 없어서 그게 유일한 방법이었다.

   그런데 그렇게 받아오는 16KB 중 스킨이 읽는 건 1KB 남짓이고, 나머지는
   스킨 머리말과 우리 씨앗(DDS_POST_META 등)이 통째로 또 실려온 것이다.
   씨앗은 fetch 로 받은 문서에서는 실행조차 안 되니 순전히 낭비다.

   진짜 페이지 이동과 이 요청은 브라우저가 확실히 구분해준다:
     주소창으로 들어옴  → Sec-Fetch-Dest: document
     스킨이 fetch 로 읽음 → Sec-Fetch-Dest: empty
   뒤쪽이면 데이터 조각만 내준다. 스킨은 DOMParser 로 파싱해서
   #tistory-data 안만 보기 때문에 이걸로 충분하다.
   (헤더가 없는 옛날 브라우저면 그냥 원래대로 통째로 내준다 — 손해 볼 게 없다) */
function isDataFetch(req) {
  if (!req || typeof req.get !== 'function') return false;
  return req.get('sec-fetch-dest') === 'empty';
}

function dataOnly(view) {
...
function page(view, opts) {
  opts = opts || {};
  if (opts.dataOnly) return dataOnly(view);
  const t = template();
  if (!t.before) {
...
<p>관리 화면은 <a href="/manage">/manage</a> 로 들어갈 수 있어.</p>`;
  ...
  const deskBits = desktopSeed() + musicSeed() + shimejiSeed() +
    galleryTitleSeed() + slotPickSeed() + winSavedSeed() + postMetaSeed() + featuredSeed() + profileSeed();
  ...
  /* 스킨은 「이 사람이 주인인가」를 window.tiara.customProps.role 로만 판정한다.
     티스토리(카카오)의 분석 객체인데, 스킨이 그걸 권한 표시로 쓰고 있어서 우리도 흉내 낸다. */
...
module.exports = { page, dataBlock, skinVar, isOn, SKIN_VARS, fmtDate, esc, boardState, isDataFetch };
