...
const cfg = {
  ...
  /* 카테고리·검색으로 들어갔을 때 기본 보기.
     'list' = 목록형 (제목·사진·본문 앞부분이 한 줄씩) / 'gallery' = 갤러리형 (사진 격자) */
  listStyle: process.env.LIST_STYLE || 'list',         // gallery | list
  galleryCategory: process.env.GALLERY_CATEGORY || '갤러리',

  sessionDays: 60,          // 들를 때마다 자동으로 연장된다 (auth.attach)
