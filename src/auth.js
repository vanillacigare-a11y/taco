...
/* 로그인할 때마다 세션이 한 줄씩 생기는데, 만료돼도 그 사람이 다시 들르기 전에는
   아무도 안 지운다. 그래서 시작할 때 한 번, 그리고 하루에 한 번 쓸어낸다.
   덤으로 한 사람이 들고 있는 세션이 너무 많이 쌓이지 않게 최근 것만 남긴다. */
const KEEP_PER_OWNER = 20;

function sweepSessions() {
  try {
    const gone = db.prepare('DELETE FROM sessions WHERE expires_at < ?').run(now()).changes;
    /* 주인 세션이 너무 많으면 오래된 것부터 정리 (기기·브라우저마다 하나면 충분하다) */
    const extra = db.prepare(
      `DELETE FROM sessions WHERE kind='owner' AND id NOT IN (
         SELECT id FROM sessions WHERE kind='owner' ORDER BY created_at DESC LIMIT ?)`
    ).run(KEEP_PER_OWNER).changes;
    if (gone || extra) console.log('세션 정리:', gone, '개 만료,', extra, '개 오래된 것');
  } catch (e) { console.error('[세션 정리]', e.message); }
}
...
function attach(req, res, next) {
  ...
  /* 반쯤 지났으면 기한을 다시 늘려준다 — 계속 드나드는 동안은 로그인이 안 풀린다 */
  if (s) {
    const full = cfg.sessionDays * DAY;
    if (s.expires_at - now() < full / 2) {
      const next = now() + full;
      db.prepare('UPDATE sessions SET expires_at=? WHERE id=?').run(next, s.id);
      s.expires_at = next;
      setCookie(res, s.id);
    }
  }
...
module.exports = {
  COOKIE, createSession, readSession, setCookie, clearCookie, sweepSessions,
