...
function bodyHtml(text) {
  /* 예전에는 올린 사진 전부를 <img> 로 여기 붙였다.
     스킨이 「저장된 사진 주소가 만료됐으면 이 목록에서 살아 있는 주소를 찾아 쓴다」는
     티스토리용 장치를 갖고 있어서였다.

     우리 사진 주소(/uploads/…)는 만료가 없어서 그 대조는 한 번도 성사되지 않았고
     (주소 형식이 달라 애초에 안 맞았다 — 콘솔에 뜨던 그 경고),
     사진 수만큼 길어지기만 했다. 페이지를 열 때마다 읽는 글이라 그냥 짐이었다.

     그래서 목록을 뺐다. 주소가 어긋나는 경우는
     bridge/desktop.js 의 fixUploadUrls() 가 화면에서 직접 고쳐 끼운다. */
  const safe = String(text || '')
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  return `<div class="slot-store"><pre class="slot-config">${safe}</pre></div>`;
}
...
/* 화면 이름들.
   이름은 슬롯 덩어리 바로 앞에 [이름] 한 줄로 들어 있다. 덩어리 속(압축된 설정)은
   건드릴 필요가 없어서, 이름만 다루는 일은 이 한 줄만 보면 된다. */
const NAME_LINE = /(^|\n)\[([^\[\]\n]+)\](\s*\n\[DDS-SLOT:)/g;
...
function names(text) {
  ...
  let m;
  while ((m = NAME_LINE.exec(raw))) out.push(m[2].trim());
  return out;
}

/* 이름 한 줄만 갈아끼운다 — 저장된 설정은 그대로 둔다 */
function rename(text, from, to) {
  const a = String(from || '').trim(), c = String(to || '').trim();
  return String(text || '').replace(NAME_LINE, (whole, pre, nm, post) =>
    nm.trim() === a ? pre + '[' + c + ']' + post : whole);
}
...
/* 예전엔 사진을 올리거나 지울 때마다 이 글의 사진 목록을 다시 만드는
   refreshImages() 를 불렀다. v0.26.0 에서 목록 자체를 뺐으니 할 일이 없어졌다.
   그런데도 사진 하나 올릴 때마다 글을 통째로 다시 쓰고 있어서 없앴다. */

module.exports = { ensure, getText, setText, setActive, names, rename, TITLE };
