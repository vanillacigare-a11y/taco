...
const skinassets = require('./skinassets');
...
/* 폰트·아이콘을 바깥 CDN 대신 우리 것으로 (skinassets.js 설명 참고) — 정적 배달보다 먼저 */
skinassets.mount(app);
/* 갖다 놓은 폰트·아이콘 자료는 우리가 안 고치니 오래 캐시해도 된다 */
app.use('/skin/images/vendor', express.static(path.join(cfg.skinDir, 'images', 'vendor'),
...
/* 목록·격자에 쓰는 작은 사진은 줄여서 내보낸다 (thumbs.js 설명 참고) — 원본 배달보다 먼저 */
thumbs.mount(app);
...
/* 브라우저는 파일 이름을 UTF-8 로 보내는데, 받는 쪽 기본값이 latin1 이라
   한글 이름이 「ìì§ì´ë…」 처럼 깨져서 들어온다.
   그러면 이름에서 한글이 전부 떨어져 나가 image-xxxxx.webp 가 되고,
   갤러리에 뜨는 원래 이름도 글자가 깨진 채로 남는다. 되돌려준다. */
function realName(n) {
  const s = String(n || '');
  if (!/[À-ÿ]/.test(s)) return s;              // 깨진 흔적이 없으면 그대로
  try {
    const back = Buffer.from(s, 'latin1').toString('utf8');
    return back.indexOf('�') === -1 ? back : s;     // 되돌렸는데 더 이상하면 원래대로
  } catch (e) { return s; }
}
...
async function saveImage(file, purpose) {
  ...
  file.originalname = realName(file.originalname);
  const base = (file.originalname || 'image').replace(/\.[^.]+$/, '')
    .replace(/[^\w가-힣.-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 40) || 'image';
  const rand = Math.random().toString(36).slice(2, 7);
  const name = `${base}-${rand}.webp`;

  /* 움직이는 그림(GIF·움직이는 WebP)은 움직임을 살려서 넣는다.
     예전에는 무조건 animated:false 로 읽어서 첫 장면만 남았다 —
     움직이는 GIF 를 올려도 멈춘 사진이 됐다.

     움직이는 그림은 세로로 이어붙인 한 장처럼 다뤄지기 때문에
     가로만 줄인다 (세로까지 걸면 장면 수만큼 곱해진 높이를 기준으로 재서 찌그러진다).
     돌려세우기(rotate)도 여러 장에는 안 먹으니 정지 그림에만 쓴다. */
  const probe = await sharp(file.buffer).metadata();
  const moving = Number(probe.pages || 1) > 1;

  let out;
  if (moving) {
    out = await sharp(file.buffer, { animated: true })
      .resize({ width: Math.min(probe.width || cfg.imageMaxSide, cfg.imageMaxSide), withoutEnlargement: true })
      .webp({ quality: 78 })
      .toBuffer({ resolveWithObject: true });
  } else {
    out = await sharp(file.buffer, { animated: false })
      .rotate()
  ...
  /* 움직이는 그림은 out.info.height 가 「장면들을 이어붙인 전체 높이」다.
     화면에 뜨는 실제 높이는 한 장면의 높이라서 다시 재서 넣는다. */
  let h = out.info.height;
  if (moving) {
    try {
      const m2 = await sharp(out.data, { animated: true }).metadata();
      if (m2.pageHeight) h = m2.pageHeight;
      else if (m2.pages > 1) h = Math.round(out.info.height / m2.pages);
    } catch (e) {}
  }
  ...
  return uploads.add({
    ...
    w: out.info.width, h: h, bytes: out.data.length,
    mime: 'image/webp',
...
api.get('/me', (req, res) => {
  res.json({
    ...
    /* 저장 공간이 볼륨에 붙어 있나 — 안 붙어 있으면 재배포 때 글·사진이 전부 사라진다.
       그동안 서버 로그에만 찍혀서 아무도 못 봤다. 관리 화면에 띄우려고 같이 보낸다. */
    onVolume: req.isOwner ? !!cfg.onVolume : undefined
  });
});
...
api.post('/uploads', auth.requireOwner, upload.array('files', 20), async (req, res) => {
  try {
    ...
    const slotName = String(req.body.slot || '').trim().slice(0, 60);
    const saved = [];
    let n = 0;
    for (const f of req.files || []) {
      ...
      let row = title && !forPost ? uploads.setTitle(one.id, title) : one;
      /* 지금 입고 있는 화면의 갤러리에 넣는다.
         화면 이름을 안 보내면 빈 값 = 모든 화면에 보인다. */
      if (!forPost && slotName) row = uploads.setSlot(row.id, slotName);
      saved.push(row);
    }
    if (!saved.length) return res.status(400).json({ error: 'noimage', message: '이미지 파일이 없어' });
    if (!forPost) { gallery.sync(); }
...
/* 화면(슬롯) 이름 바꾸기.
   스킨에는 이름을 고치는 길이 없어서, 이름이 맘에 안 들면 새로 만드는 수밖에 없었다.
   이름은 저장된 설정 덩어리 바깥에 [이름] 한 줄로 있어서, 그 줄만 갈아끼우면 된다.
   이름을 쓰는 다른 것들(사진이 어느 화면 갤러리에 속하는지, 기본 화면)도 같이 옮긴다. */
api.post('/slot/rename', auth.requireOwner, (req, res) => {
  const b = req.body || {};
  const from = String(b.from || '').trim();
  const to = String(b.to || '').trim();

  if (!from || !to) return res.status(400).json({ error: 'bad', message: '이름이 비었어' });
  if (to.length > 40) return res.status(400).json({ error: 'long', message: '이름이 너무 길어 (40자까지)' });
  if (/[[\]\n\r]/.test(to)) return res.status(400).json({ error: 'char', message: '이름에 [ 나 ] 는 못 써' });

  const text = slot.getText();
  if (!text.trim()) return res.status(404).json({ error: 'none', message: '저장된 화면이 없어' });

  const have = slot.names(text);
  if (have.indexOf(from) === -1) {
    return res.status(404).json({ error: 'nofrom', message: '「' + from + '」 라는 화면이 없어' });
  }
  if (from === to) return res.json({ ok: true, from, to, same: true });
  if (have.indexOf(to) !== -1) {
    return res.status(409).json({ error: 'dup', message: '「' + to + '」 는 이미 있는 이름이야' });
  }

  const next = slot.rename(text, from, to);
  if (next === text) return res.status(500).json({ error: 'nochange', message: '이름을 못 바꿨어' });
  slot.setText(next);

  /* 이름을 쓰던 것들도 따라 옮긴다 */
  try { uploads.renameSlot(from, to); } catch (e) {}
  try { if (settings.get('board:slot', '') === from) settings.set('board:slot', to); } catch (e) {}

  /* 화면마다 따로 기억하는 잠금도 이름을 쓴다 */
  try {
    const raw = settings.get('desktop:state', '');
    if (raw) {
      const st = JSON.parse(raw);
      if (st && st.locks && st.locks[from]) {
        delete st.locks[from];
        st.locks[to] = true;
        settings.set('desktop:state', JSON.stringify(st));
      }
    }
  } catch (e) {}

  res.json({ ok: true, from, to, names: slot.names(next) });
});
...
api.post('/uploads/remove', auth.requireOwner, (req, res) => {
  ...
  /* 화면에 뜬 주소(raw)는 한글이 %EB%B4%84… 로 바뀌어 있을 수 있다. 둘 다 찾아본다. */
  const u = uploads.byPath(p) || (b.raw ? uploads.byPath(String(b.raw)) : null);
  if (!u) {
    return res.status(404).json({
      error: 'notfound',
      message: '목록에 없는 사진이야 (' + p.split('/').pop() + ')'
    });
  }
...
api.put('/slot', auth.requireOwner, (req, res) => {
  ...
  /* 한 번이라도 화면을 저장했으면, 창 크기는 이제 주인이 정한 것이다 —
     들어올 때 우리가 블로그 창을 기본 크기로 다시 잡으면 안 된다 */
  try { settings.set('board:winSaved', '1'); } catch (e) {}
...
api.put('/desktop', auth.requireOwner, (req, res) => {
  ...
  /* 블로그 창 크기·자리 — 스킨은 위젯 창만 슬롯에 담아서, 이건 우리가 챙긴다 */
  const win = (b.win && typeof b.win === 'object' && !Array.isArray(b.win)) ? b.win : {};
  const cleanWin = {};
  for (const [k, v] of Object.entries(win)) {
    if (!/^[\w:.-]{1,40}$/.test(k) || !v || typeof v !== 'object') continue;
    const box = {};
    for (const f of ['w', 'h', 'x', 'y']) {
      const n = Number(v[f]);
      if (Number.isFinite(n) && n >= 0 && n < 20000) box[f] = Math.round(n);
    }
    /* 다시 띄울 때 바탕화면 아이콘을 찾아 누르려면 창 이름이 필요하다 */
    if (typeof v.t === 'string' && v.t.trim()) box.t = v.t.trim().slice(0, 40);
    if (box.w && box.h) cleanWin[k] = box;
  }

  /* 화면(슬롯)마다 「스티커·아이콘 자리 잠금」 — { 화면이름: true } 만 담는다 */
  const locks = (b.locks && typeof b.locks === 'object' && !Array.isArray(b.locks)) ? b.locks : {};
  const cleanLocks = {};
  for (const [k, v] of Object.entries(locks)) {
    if (!v) continue;                       // 풀린 건 아예 적지 않는다
    const name = String(k).slice(0, 60);
    if (!name) continue;
    cleanLocks[name] = true;
  }

  const payload = JSON.stringify({
    keys: clean, icons: cleanIcons, theme: cleanTheme, win: cleanWin, locks: cleanLocks
  });
...
/* 페이지를 내줄 때 붙이는 것들.
   dataOnly = 스킨이 화면에 그리려고 데이터만 뽑아가는 요청 (render.js 설명 참고) */
function pageOpts(req) {
  return { owner: req.isOwner, guest: req.guestName, dataOnly: render.isDataFetch(req) };
}

app.get('/', (req, res) => {
  res.type('html').send(render.page(listView(req, { bodyId: 'tt-body-index' }), pageOpts(req)));
});
...
app.use('/tag', (req, res, next) => {
  ...
  if (!tag) return res.type('html').send(render.page(listView(req, { bodyId: 'tt-body-tag', title: '태그' }), pageOpts(req)));
  res.type('html').send(render.page(listView(req, { tag, bodyId: 'tt-body-tag', title: '태그: ' + tag }), pageOpts(req)));
});
...
app.get('/:id', (req, res, next) => {
  ...
  res.type('html').send(render.page({
    ...
    /* 지금 입고 있는 화면 — 갤러리 저장소 글을 이 화면 것만 남기고 거른다 */
    wearing: String((req.cookies && req.cookies.dds_slot) || '')
  }, pageOpts(req)));
});
...
if (require.main === module) {
  ...
  slot.ensure();
  skinassets.check();
  skinassets.checkHook();
  /* 만료된 세션 쓸어내기 — 시작할 때 한 번, 그다음엔 하루에 한 번 */
  auth.sweepSessions();
  const sweep = setInterval(auth.sweepSessions, 24 * 60 * 60 * 1000);
