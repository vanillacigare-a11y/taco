...
const uploads = {
  add(u) {
    const info = db.prepare(`INSERT INTO uploads(path,orig_name,w,h,bytes,mime,purpose,title,created_at)
      VALUES(@path,@orig_name,@w,@h,@bytes,@mime,@purpose,@title,@created_at)`)
      .run({ purpose: 'gallery', title: '', ...u, created_at: now() });
